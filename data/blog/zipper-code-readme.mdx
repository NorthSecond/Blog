---
title:  "Precision-guided context sensitivity for pointer analysis 代码复现记录"
date:   2023-01-17 22:00:00 +0800
summary: "课程[软件源代码分析]小组作业的个人部分。"
tags: [Notes, 课程内容]
---
...
本项目的Docker file在[这里](https://github.com/NorthSecond/zipper/blob/master/dockerfile)下载，放置在项目根目录即可。
...
x 1#include <bits/stdc++.h>2​3using namespace std;4​5typedef long long ll;6typedef unsigned long long ull;7typedef unsigned int usint;8​9#define fre(x, i) for(i = 0; i < x; ++i)10​11​12#define frp(i, a, b) for(i = a; i < b; ++i)13​14int main(void) {15    ios::sync_with_stdio(false);16    cin.tie(NULL);17    18    string origin = "";19    string ans = "";20    string line, tmp;21​22    usint s, i, j, k, lines;23    cin >> s;24    25    lines = s / 8;26    if(s % 8) {27        lines++;28    }29    fre(lines, i){30        cin >> line;31        origin += line;32    }33​34    // suoyin35    stack<usint> len_vs;36    usint index = 0;37    string ops = origin.substr(index, 2);38    usint op = strtoul(ops.c_str(), NULL, 16);39​40    while(op > 127) {41        len_vs.push(op - 128);42        index += 2;43        ops = origin.substr(index, 2);44        op = strtoul(ops.c_str(), NULL, 16);45    }46    len_vs.push(op);47    index += 2;48​49    // datas50    while(index < 2 * s) {51        ops = origin.substr(index, 2);52        op = strtoul(ops.c_str(), NULL, 16);53        index += 2;54        unsigned opv = op % 4;55        if(opv == 0) {56            usint l = op / 4;57            if (l<60){58                // l = l + 159                ++l;60            } else if(l == 60){61                // 1 byte62                tmp = origin.substr(index, 2);63                l = strtoul(tmp.c_str(), NULL, 16) + 1;64                index += 2;65            } else if (l == 61) {66                // 2 byte67                tmp = "";68                tmp += origin[index + 2];69                tmp += origin[index + 3];70                tmp += origin[index];71                tmp += origin[index + 1];72                l = strtoul(tmp.c_str(), NULL, 16) + 1;73                index += 4;74            } else if (l == 62) {75                // 3 byte76                tmp = "";77                tmp += origin[index + 4];78                tmp += origin[index + 5];79                tmp += origin[index + 2];80                tmp += origin[index + 3];81                tmp += origin[index];82                tmp += origin[index + 1];83                l = strtoul(tmp.c_str(), NULL, 16) + 1;84                index += 6;85            } else if(l == 63) {86                // 4 byte87                tmp = "";88                tmp += origin[index + 6];89                tmp += origin[index + 7];90                tmp += origin[index + 4];91                tmp += origin[index + 5];92                tmp += origin[index + 2];93                tmp += origin[index + 3];94                tmp += origin[index];95                tmp += origin[index + 1];96                l = strtoul(tmp.c_str(), NULL, 16) + 1;97                index += 8;98            }99            ans += origin.substr(index, l * 2);100            index += 2 * l;101        } else if(opv == 1) {102            size_t oh3 = op / 32;103            size_t l = op % 32;104            l /= 4;105            l += 4;106            tmp = origin.substr(index, 2);107            size_t ol = strtoul(tmp.c_str(), NULL, 16);108            index += 2;109            size_t o = oh3 * 256 + ol;110​111            // Repeat<o, l>112            usint now_ans_size = ans.length();113            // debug("1: Now ans size = %lu\n o = %u, l = %u\n", now_ans_size, o, l);114            if(l <= o) {115                ans += ans.substr(now_ans_size - o * 2, l * 2);116            } else {117                usint repeat_times = l / o;118                tmp = ans.substr(now_ans_size - o * 2, o * 2);119                fre(repeat_times, i) {120                    ans += tmp;121                }122                if(l % o) {123                    ans += tmp.substr(0, (l % o) * 2);124                }125            }126        } else if(opv == 2) {127            size_t l = op / 4 + 1;128            tmp = "";129            tmp += origin[index + 2];130            tmp += origin[index + 3];131            tmp += origin[index];132            tmp += origin[index + 1];133            size_t o = strtoul(tmp.c_str(), NULL, 16);134            index += 4;135            // Repeat<o, l>136            137            usint now_ans_size = ans.length();138            // debug("2: Now ans size = %lu\n o = %u, l = %u\n", now_ans_size, o, l);139​140            if(l <= o) {141                ans += ans.substr(now_ans_size - o * 2, l * 2);142            } else {143                usint repeat_times = l / o;144                tmp = ans.substr(now_ans_size - o * 2, o * 2);145                fre(repeat_times, i) {146                    ans += tmp;147                }148                if(l % o) {149                    ans += tmp.substr(0, (l % o) * 2);150                }151            }152        } else {153            // ERROR154            puts("ERROR");155            assert(true);156        }157​158    }    159​160    // print ans161    usint now_ans_size = ans.length();162    fre(now_ans_size, i) {163        if(i && i % 16 == 0) {164            cout << '\n';165        }166        cout << ans[i];167    }168    cout << '\n';169    return 0;170}171​cpp
...
![image-20230117141248376](/img/post_pics/image-20230117141248376.png)
...
![image-20230117135412200](/img/post_pics/image-20230117135412200.png)
...
![image-20230117135619117](/img/post_pics/image-20230117135619117.png)
...
![image-20230117144109900](/img/post_pics/image-20230117144109900.png)
