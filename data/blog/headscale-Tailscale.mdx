...
[mdx_fold open="true" title="配置文件详情"]
```yaml
...
访问 `<server_url>/windows` 可以查到对应的接入方法。具体而言就是在下载好新版本的 tailscale 之后在 powershell 中运行：
...
```bash
tailscale up --login-server=<server_url> --accept-routes=true --accept-dns=false --reset
```
按照指引安装即可。
...
```bash
tailscale up --login-server=<server_url> --accept-routes=true --accept-dns=false --authkey <KEY>
```
